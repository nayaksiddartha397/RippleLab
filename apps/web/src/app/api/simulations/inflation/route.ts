...
export async function POST(request: Request) {
  ...
  try {
    const response = await fetch(`${engineUrl}/v1/simulations/inflation`, {
      ...
      return Response.json(
        {
          error: {
            ...
            message: errorMessage(payload, "Check the scenario values and try again."),
          },
        },
    ...
    return Response.json(
      {
        error: {
          ...
          message: "The economic engine could not be reached. Please try again shortly.",
        },
      },
      { status: 503 },
    );
